...
use crate::{
    ...
    ScheduleStatus, ScheduleType, SchedulerConfig, SchedulerEvent,
};

// ============================================================================
// Schedule Service Trait
// ============================================================================
...
pub struct DurableScheduler<S: ScheduleService> {
    store: Arc<S>,
...
impl<S: ScheduleService + 'static> DurableScheduler<S> {
...
struct SchedulerTaskHandle<S: ScheduleService> {
    store: Arc<S>,
    config: SchedulerConfig,
...
impl<S: ScheduleService + 'static> SchedulerTaskHandle<S> {
    fn emit(&self, event: SchedulerEvent) {
        let _ = self.event_sender.send(event);
    }
