...
pub async fn send_message(
    ...
    let url = format!("{}/teams/{}/channels/{}/messages", GRAPH_API_BASE, team_id, channel_id);
    ...
    let json: Value = serde_json::from_str(&response_body).map_err(|e| format!("Failed to parse response: {}", e))?;
...
pub async fn create_channel(
    ...
    let web_url = json.get("webUrl").and_then(|v| v.as_str()).map(|s| s.to_string());
...
pub async fn delete_channel(
    ...
    let client = create_client(30)?;
    let url = format!("{}/teams/{}/channels/{}", GRAPH_API_BASE, team_id, channel_id);
...
pub async fn get_channel_messages(
    ...
    let client = create_client(30)?;
    let mut url = format!("{}/teams/{}/channels/{}/messages", GRAPH_API_BASE, team_id, channel_id);
    ...
    Ok(GetChannelMessagesOutput { messages, next_link })
}
...
pub async fn get_team(access_token: &str, team_id: &str) -> Result<GetTeamOutput, String> {
    let client = create_client(30)?;
    ...
    if !status.is_success() {
        return Err(format!("Failed to get team: {}", extract_error_message(&response_body)));
    }
...
pub async fn list_teams(access_token: &str, filter: Option<&str>, top: Option<i32>) -> Result<ListTeamsOutput, String> {
    let client = create_client(30)?;
...
pub async fn remove_team_member(
    ...
    let client = create_client(30)?;
    let url = format!("{}/teams/{}/members/{}", GRAPH_API_BASE, team_id, member_id);
