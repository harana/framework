...
use aws_sdk_s3::{
    ...
    types::{
        BucketLocationConstraint, BucketVersioningStatus,
        CompletedMultipartUpload, CompletedPart, CorsRule, CreateBucketConfiguration,
        Delete, LifecycleRule, ObjectIdentifier,
...
pub async fn delete_objects(
    ...
    let deleted: Vec<HashMap<String, Value>> = response
        ...
        .map(|d| {
            ...
            map.insert("key".to_string(), Value::String(d.key().unwrap_or_default().to_string()));
    ...
    let errors: Vec<HashMap<String, Value>> = response
        ...
        .map(|e| {
            let mut map = HashMap::new();
            map.insert("key".to_string(), Value::String(e.key().unwrap_or_default().to_string()));
            map.insert("code".to_string(), Value::String(e.code().unwrap_or_default().to_string()));
            map.insert("message".to_string(), Value::String(e.message().unwrap_or_default().to_string()));
            map
        })
        .collect();
    ...
    let rules: Vec<CorsRule> = cors_rules
        ...
        .map(|rule| {
            ...
            let mut builder = CorsRule::builder()
                .set_allowed_headers(if allowed_headers.is_empty() { None } else { Some(allowed_headers) })
                .set_allowed_methods(Some(allowed_methods))
                .set_allowed_origins(Some(allowed_origins))
                .set_expose_headers(if expose_headers.is_empty() { None } else { Some(expose_headers) });
...
pub async fn put_object(
    ...
    if let Some(sse) = server_side_encryption {
        request = request.server_side_encryption(
            sse.parse().map_err(|_| "Invalid server-side encryption")?,
        );
    }
