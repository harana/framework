...
impl Action {
    ...
    fn parse_field(value: &serde_yaml::Value) -> Result<Field> {
        let value_str = value
            .as_str()
            .context("Field value must be a string")?
            .trim();
...
pub fn parse_action_schemas(schema_dir: &Path, module_filter: Option<&str>) -> Result<Vec<ActionSchema>> {
        ...
        .filter_map(|e| e.ok())
    {
        ...
        let actions_yaml: Vec<serde_yaml::Value> = serde_yaml::from_str(&content)
            .with_context(|| format!("Failed to parse YAML from {}", path.display()))?;

        let mut actions = Vec::new();

        for action_yaml in actions_yaml {
            let mapping = action_yaml
                .as_mapping()
            ...
            let name = mapping
                .get("name")
                .and_then(|v| v.as_str())
                .context("Action missing 'name' field")?
                .to_string();

            let class = mapping
                .get("class")
                .and_then(|v| v.as_str())
                .context("Action missing 'class' field")?
                .to_string();
