#!/usr/bin/env python3
"""
Enhanced refactor script to remove:
1. Field comments (/// comments before struct fields)
2. Enum variant comments (/// comments before enum variants)
3. Move inline test modules to separate tests.rs files
"""
...
def remove_all_inner_comments(content: str) -> str:
    """
    Remove /// comments that appear before struct fields and enum variants.
    Keep top-level doc comments for structs, enums, functions, etc.
    """
    lines = content.split('\n')
    result_lines = []
    i = 0
    ...
    while i < len(lines):
        ...
        # Track if we're inside a struct or enum definition
        if re.search(r'\b(struct|enum)\s+\w+', line):
            in_struct_or_enum = True
        
        # Track braces
        brace_depth += line.count('{') - line.count('}')
        
        if brace_depth == 0 and in_struct_or_enum:
            in_struct_or_enum = False
        ...
        if re.match(r'^\s*///', line):
            ...
            # Check if we found a field or variant definition
            is_field_or_variant = False
            if k < len(lines):
                next_line = lines[k].strip()
                # Inside a struct or enum
                if in_struct_or_enum or brace_depth > 0:
                    # Match field patterns like:
                    # pub field: Type,
                    # pub(crate) field: Type,
                    # field: Type,
                    if re.match(r'^(pub(\s*\([^)]+\))?\s+)?\w+\s*:', next_line):
                        is_field_or_variant = True
                    # Match enum variant patterns like:
                    # VariantName,
                    # VariantName(Type),
...
def process_file(file_path: str) -> bool:
    ...
    try:
        ...
        # Step 1: Remove field and variant comments
        content_no_comments = remove_all_inner_comments(content)
        if content_no_comments != content:
            modified = True
            content = content_no_comments
            print(f"  • Removed field/variant comments")
...
def main():
    if len(sys.argv) < 2:
        print("Usage: refactor_v3.py <directory>")
        sys.exit(1)
